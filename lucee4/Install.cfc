<cfcomponent displayname="Install" output="false">
  <!--- Extension name --->
  <cfset variables.name = "cflibicalrecurrence" />
  ...
  <cffunction name="install" access="public" returntype="string" output="false">
    ...
    <!--- Add components --->
    <cfdirectory action="create" directory="#serverPath#/components/libicalrecurrence" />
    <cfset dirPath = arguments.path & "components" />
    <cfdirectory action="list" directory="#dirPath#" type="file" filter="*.cfc" name="dirContent">
    <cfloop query="dirContent">
      <cffile action="copy"
              source="#dirPath#/#dirContent.name#"
              destination="#serverPath#/components/libicalrecurrence/#dirContent.name#"
      />
    </cfloop>
  ...
  <cffunction name="uninstall" access="public" returntype="string" output="false">
    ...
    <cfdirectory action="list" directory="#serverPath#/library/function" filter="*libicalrecurrence*" name="dirContent" />
    <cfloop query="dirContent">
      <cfset removeFile("#serverPath#/library/function/#dirContent.name#", errors) />
    </cfloop>

    <!--- Remove the components --->
    <cfdirectory action="list" directory="#serverPath#/components/libicalrecurrence" name="dirContent" />
    <cfloop query="dirContent">
      <cfset removeFile("#serverPath#/components/libicalrecurrence/#dirContent.name#", errors) />
    </cfloop>
    <cfdirectory action="delete" directory="#serverPath#/components/libicalrecurrence" />
